...
const EditPlaylist : FC = () => {
  ...
  return (
    <div className="relative w-full h-full border border-white/20 rounded-md p-6 flex justify-center items-center">
      ...
      <Link to={`/playlists/${id}`} className=" absolute top-6 left-5 uppercase italic hover:text-white/80">
        <IoIosArrowBack className="inline text-2xl -mt-1" />Playlist
      </Link>

      <form onSubmit={handleEdit} className="w-10/12 max-w-md mb-20">

        <div className="text-center mb-5">
          <h1 className="text-4xl font-bold mb-2 text-spotify-green">Edit playlist</h1>
          <p className="text-lg text-white">Edit <span className="text-spotify-green">{title}</span> playlist.</p>
        </div>
        ...
        <div className="mb-5">
          <button className={`${isLoading ? "bg-spotify-greendark" : "bg-spotify-green"} text-white shadow-md text-xl px-4 py-2 rounded-md leading-tight hover:bg-spotify-greendark hover:font-bold w-full transition-all duration-700`}>{isLoading ? "Loading..." : "Edit playlist"}</button>
        </div>
