...
const PlaylistCard : FC<{ p : Playlist }> = ({ p }) => (
  <Link
    to={`/playlists/${p._id}`}
    className="m-4 md:w-[400px] h-28 md:h-40 flex items-center border border-white/20 rounded-md text-center hover:bg-white/20 cursor-pointer transition-all duration-700">

    <div className="w-28 md:w-40 shrink-0">
      <PlaylistImage tracks={p.tracks} customClasses="w-28 h-28 md:w-40 md:h-40 rounded-l-md" />
    </div>
    ...
    <div className="flex flex-col justify-center w-full h-full relative overflow-hidden p-1">
      ...
      <h1 className="w-full px-2 font-bold text-spotify-green md:text-lg shrink-0">
        {p.title}
        {p.isPublic
